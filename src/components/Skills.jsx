...
function SkillBar({ name, pct, visible, delay }) {
  ...
  return (
    <div className="mb-5">
      <div className={`flex justify-between text-sm mb-2 ${isDark ? '' : ''}`}>
        <span className={`text-xs tracking-wide ${isDark ? 'text-[#E8E4D9]' : 'text-[#1a1208] font-medium'}`}>{name}</span>
        <span className="font-mono text-[#C9A84C] text-xs">{pct}%</span>
      </div>
      <div className={`h-px rounded-full overflow-hidden relative ${isDark ? 'bg-[#111D33]' : 'bg-[#e0dbd3]'}`}>
        <div className={`absolute inset-0 ${isDark ? 'bg-[#111D33]' : 'bg-[#e0dbd3]'}`} />
        <div
  ...
  return (
    <div
      ...
      className={`border rounded-2xl p-8 hover:border-[#C9A84C]/30 transition-all duration-500 ${
        isDark ? 'bg-[#0D1526] border-[#C9A84C]/10' : 'bg-white border-[#C9A84C]/20 shadow-sm'
      } ${
    ...
    >
      <div className="flex items-center gap-3 mb-7">
        <span className={`font-mono text-xs tracking-[0.2em] uppercase ${isDark ? 'text-[#C9A84C]/70' : 'text-[#555]/70'}`}>{group.title}</span>
        <div className={`flex-1 h-px ${isDark ? 'bg-[#C9A84C]/10' : 'bg-[#C9A84C]/15'}`} />
      </div>
