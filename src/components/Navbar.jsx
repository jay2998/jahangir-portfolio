...
export default function Navbar({ darkMode, setDarkMode }) {
  ...
  const navBg = darkMode
    ...
    : scrolled ? 'bg-white/98 border-[#C9A84C]/40 shadow-md' : 'bg-white/95 border-[#C9A84C]/30'
      ...
      <ul className="hidden md:flex items-center gap-7">
          ...
          <li key={link}>
            <a
              ...
              className={`font-mono text-xs tracking-widest uppercase transition-all duration-300 hover:text-[#C9A84C] ${
                active === link ? 'text-[#C9A84C]' : darkMode ? 'text-[#7A8099]' : 'text-[#222]'
              }`}
            >
              {link}
        ...
        <li className="flex items-center gap-3 pl-4 border-l border-[#C9A84C]/15">
          <a href={personalInfo.linkedin} target="_blank" rel="noreferrer"
            className={`transition-colors duration-300 hover:text-[#C9A84C] ${darkMode ? 'text-[#7A8099]' : 'text-[#222]'}`} aria-label="LinkedIn">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
            </svg>
          </a>
          <a href={personalInfo.github} target="_blank" rel="noreferrer"
            className={`transition-colors duration-300 hover:text-[#C9A84C] ${darkMode ? 'text-[#7A8099]' : 'text-[#222]'}`} aria-label="GitHub">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
        ...
        <li>
          <button
            ...
            className={`w-9 h-9 rounded-full border flex items-center justify-center transition-all duration-300 hover:border-[#C9A84C]/60 ${
              darkMode ? 'border-[#C9A84C]/20 text-[#C9A84C]/70 hover:text-[#C9A84C]' : 'border-[#C9A84C]/50 text-[#C9A84C] hover:bg-[#C9A84C]/15'
            }`}
        ...
        <div className={`absolute top-full left-0 right-0 border-b border-[#C9A84C]/20 flex flex-col py-4 md:hidden ${darkMode ? 'bg-[#0D1526]' : 'bg-white'}`}>
          {links.map(link => (
            <a key={link} href={`#${link}`} onClick={() => setMenuOpen(false)}
              className={`font-mono text-xs px-8 py-3 tracking-widest uppercase transition-colors hover:text-[#C9A84C] ${darkMode ? 'text-[#7A8099]' : 'text-[#222]'}`}>
              {link}
            </a>
          ))}
          <div className="flex items-center gap-4 px-8 pt-4 mt-2 border-t border-[#C9A84C]/10">
            ...
            <a href="/cv/Jahangir_Ali_CV_Pakistan.pdf" download className="ml-auto font-mono text-xs text-[#C9A84C] border border-[#C9A84C]/40 px-3 py-1.5 rounded hover:bg-[#C9A84C] hover:text-[#080E1C] transition-all">↓ CV</a>
          </div>
        </div>
      )}
    </nav>
